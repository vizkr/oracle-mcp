...
Your user asks a question that is not a lookup. Should I take this job. Is this the year. What does this dream mean. When they want counsel rather than data, or when they explicitly ask for a reading, tarot, Hafez, the I Ching, runes, or geomancy. Route by intent:

| Tool | Use when | Character |
|---|---|---|
| `recommend_oracle` | The question is ambiguous and you do not know which instrument fits | Returns the best-matching oracle with reasoning |
...
4. **Privacy by default.** Questions and readings are not persisted. A future `visibility: "public"` opt-in will let callers contribute featureable readings; absent that flag, nothing is stored.
